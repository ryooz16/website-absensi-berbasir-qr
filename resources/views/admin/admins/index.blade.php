...
<div x-data="{ 
    ...
    <div class="p-4 md:p-6 lg:p-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
                ...
                <p class="text-sm text-slate-400 mt-1">Kelola akun dengan akses Administrator sistem.</p>
            </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="ml-2 text-[9px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded font-bold uppercase">Admin</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center gap-1">
                                    ...
                                    @if($totalAdmin > 1)
                                        ...
                                        <button @click="$dispatch('confirm-dialog', { title: 'Hapus Admin?', message: 'Akses masuk {{ addslashes($admin->name) }} akan dicabut secara permanen.', confirmText: 'Ya, Hapus', type: 'danger', formId: 'delete-admin-{{ $admin->id }}' })" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/></svg>
                                        </button>
                                    @else
                                        <span class="p-2 text-slate-300 cursor-not-allowed" title="Admin terakhir tidak bisa dihapus">
                ...
                @forelse($admins as $admin)
                    <div class="p-4 hover:bg-slate-50 transition active:bg-slate-100">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[9px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded font-bold uppercase shrink-0">Admin</span>
                                </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            @if($totalAdmin > 1)
                                <button @click="$dispatch('confirm-dialog', { title: 'Hapus Admin?', message: 'Akses masuk {{ addslashes($admin->name) }} akan dicabut secara permanen.', confirmText: 'Ya, Hapus', type: 'danger', formId: 'delete-admin-{{ $admin->id }}' })"
    ...
    <x-modal name="tambah-admin" :show="old('_method') !== 'PUT' && $errors->any()" focusable>
        <div class="p-6">
            <h2 class="text-lg font-bold text-slate-800 mb-1">Tambah Admin Baru</h2>
            <p class="text-sm text-slate-400 mb-6">Buat akun administrator baru.</p>
            <form action="{{ route('admin.admins.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama administrator" class="w-full border border-slate-200 rounded-xl p-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition @error('name') border-red-400 @enderror">
    ...
    <x-modal name="edit-admin" :show="old('_method') === 'PUT' && $errors->any()" focusable>
        <div class="p-6">
            <h2 class="text-lg font-bold text-slate-800 mb-1">Edit Data Admin</h2>
            <p class="text-sm text-slate-400 mb-6">Perbarui informasi administrator.</p>
